...
def categorical_preprocessing(data):
    ...
    for i in tqdm(categories):
        ...
        mode_group_cano = data.groupby("cano")[i].transform(get_mode)
        mode_group_chid = data.groupby("chid")[i].transform(get_mode)
        data[f"mode_group_{i}"] = mode_group_cano
        data[f"mode_group_{i}_2"] = mode_group_chid
        data[f"sum_cano_{i}"] = data.groupby("cano")[i].transform('nunique')
        data[f"sum_chid_{i}"] = data.groupby("chid")[i].transform('nunique')
